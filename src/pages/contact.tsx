  ...
  >
    ...
    <div className="max-w-5xl w-full text-center space-y-8 lg:space-y-12 relative z-10">
      ...
      <div className="space-y-4 lg:space-y-6">
        <JackInTheBox delay={200} triggerOnce>
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span className="text-blue-300 text-sm font-medium tracking-wider uppercase">Get In Touch</span>
            <HiSparkles className="w-7 h-7 lg:w-8 lg:h-8 text-purple-400 animate-pulse delay-300" />
          </div>
          <h2 className="text-5xl sm:text-6xl lg:text-7xl font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent tracking-tight">
            Let's Connect!
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mt-8 lg:mt-12">
        {contactLinks.map(({ href, label, Icon, gradient, description, bgGradient }, index) => (
          <Zoom key={label} delay={400 + index * 100} duration={700} triggerOnce>
            <a
              href={href}
              target="_blank"
            ...
            >
              ...
              <div className={`absolute -inset-1 bg-gradient-to-r ${bgGradient} rounded-2xl blur opacity-25 group-hover:opacity-75 transition duration-1000`}></div>
              ...
              <div className="relative h-full p-8 lg:p-10 bg-gray-800/60 backdrop-blur-md rounded-2xl border border-gray-700/50 hover:border-gray-600 transition-all duration-300 flex flex-col items-center justify-center">
                {/* Icon */}
                <div className={`p-5 bg-gradient-to-br ${gradient} rounded-2xl shadow-xl mb-6 transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}>
                ...
                {/* Bottom accent line */}
                <div className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${gradient} rounded-b-2xl transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left`}></div>
              </div>
            </a>
          </Zoom>
        ))}
