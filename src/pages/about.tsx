		...
		<div className="max-w-6xl w-full text-center space-y-16 relative z-10">
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
				...
				<InfoCard
					...
					icons={[
						{
							icon: <span className="w-full h-full text-2xl">🏃</span>,
...
const InfoCard = ({
	...
	const borderColors =
		primaryColor === "blue" ? "border-blue-500/30" : "border-purple-500/30";
		...
		<Fade delay={delay} triggerOnce>
			<Slide direction="up" duration={800} triggerOnce>
				<div className="group relative h-full">
					...
					<div
						className={`absolute -inset-1 bg-gradient-to-r ${gradientColors} rounded-2xl blur opacity-25 group-hover:opacity-75 transition duration-1000`}
					></div>

					<div
						className={`relative h-full p-8 bg-gray-800/60 backdrop-blur-md rounded-2xl border ${borderColors} hover:border-opacity-60 transition-all duration-300`}
					>
						{/* Header */}
						<div className="text-left mb-6">
							...
							<div
								className={`w-12 h-1 bg-gradient-to-r ${primaryColor === "blue" ? "from-blue-500 to-cyan-500" : "from-purple-500 to-pink-500"} rounded-full`}
							></div>
						</div>
								...
								<Fade delay={delay + 200} cascade damping={0.1}>
									{icons.map((item, index) => (
										<div key={index} className="group/icon relative">
											<div
												className={`p-4 bg-gradient-to-br ${item.color} rounded-xl shadow-lg hover:shadow-xl transform hover:scale-110 transition-all duration-300 flex items-center justify-center h-16 w-16 mx-auto`}
											>
