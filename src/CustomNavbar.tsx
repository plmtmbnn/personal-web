...
const navItems = [
  { to: '/', icon: TbHomeHeart, label: 'Home', color: 'from-blue-500 to-cyan-500' },
  { to: '/about', icon: FaUserNinja, label: 'About', color: 'from-purple-500 to-pink-500' },
  { to: '/portfolio', icon: GrDeploy, label: 'Portfolio', color: 'from-indigo-500 to-blue-500' },
  { to: '/adventures', icon: TbActivityHeartbeat, label: 'Adventures', color: 'from-emerald-500 to-teal-500' },
  { to: '/blog', icon: PiArticleNyTimesLight, label: 'Blog', color: 'from-orange-500 to-red-500' },
  { to: '/contact', icon: FaRegAddressCard, label: 'Contact', color: 'from-cyan-500 to-blue-500' }
];
        ...
        <div className="relative flex gap-1 sm:gap-2 bg-gray-800/90 backdrop-blur-xl border border-gray-700/50 shadow-2xl p-2 rounded-2xl">
          {navItems.map(({ to, icon: Icon, label, color }) => {
            ...
            return (
              ...
              >
                {/* Button container */}
                <div className={`relative flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-xl transition-all duration-300
                  ${
              isActive 
                ? 'scale-110' 
                ...
                >
                  ...
                  {isActive && (
                    <div className={`absolute inset-0 bg-gradient-to-br ${color} rounded-xl shadow-lg`}></div>
                  )}
                ...
                {/* Hover glow effect */}
                {!isActive && (
                  <div className={`absolute inset-0 bg-gradient-to-br ${color} rounded-xl opacity-0 group-hover/item:opacity-20 transition-opacity duration-300`}></div>
                )}
              </Link>
            );
          })}
