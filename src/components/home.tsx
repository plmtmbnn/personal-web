    ...
    <div className="text-center w-full max-w-2xl relative z-10 space-y-6 sm:space-y-8 px-2">
      <JackInTheBox delay={200} triggerOnce>
        <div className="inline-block animate-soft-bounce">
          ...
          <div className="rounded-full overflow-hidden w-24 h-24 sm:w-24 sm:h-24 shadow-md">
            <img src={profileImage} alt="Polma Tambunan" className="object-cover w-full h-full" />
          </div>
        </div>
      </JackInTheBox>
      ...
      <Fade delay={300} cascade damping={0.1} triggerOnce>
        <div className="space-y-4 sm:space-y-6">
          ...
          <Slide direction="down" delay={200} duration={800} triggerOnce>
            ...
            Compiling innovations, clocking distances <br />– where sprint reviews meet actual sprints.
            </p>
          </Slide>
          

          <Slide direction="up" duration={800} triggerOnce>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-2 mt-2 sm:mt-2">
              <StatCard
                icon={<GrDeploy className="w-6 h-6 sm:w-6 sm:h-6 text-sky-600" />}
                title={`${duration}+ Years`}
                subtitle="Development"
              />
              <StatCard
                icon={<LiaRunningSolid className="w-6 h-6 sm:w-6 sm:h-6 text-emerald-500" />}
                title="1000+ KM"
                subtitle="Running per year"
              />
              <StatCard
                icon={<FiCamera className="w-6 h-6 sm:w-6 sm:h-6 text-indigo-500" />}
                title="Capture"
